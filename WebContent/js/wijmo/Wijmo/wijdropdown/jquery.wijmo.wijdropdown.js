    ...
    */
    (function (dropdown) {
        ...
        var wijdropdown = (function (_super) {
            ...
            wijdropdown.prototype._innerDisable = function () {
                _super.prototype._innerDisable.call(this);
                this._toggleDisableDropDown(true);
            };

            wijdropdown.prototype._innerEnable = function () {
                _super.prototype._innerEnable.call(this);
                this._toggleDisableDropDown(false);
            };

            wijdropdown.prototype._toggleDisableDropDown = function (disabled) {
                var wijCSS = this.options.wijCSS;
                this._labelWrap.toggleClass(wijCSS.stateDisabled, disabled);
                this._label.toggleClass(wijCSS.stateDisabled, disabled);
                this.element.attr("disabled", disabled ? "disabled" : "");
            };
            ...
            wijdropdown.prototype._createSelect = function () {
                this.cssWidth = this.element.uniqueId().get(0).style.width;
                var self = this, o = self.options, wijCSS = self.options.wijCSS, ele = self.element, width = ele.width(), eleWidth = width, selectWrap = ele.wrap("<div></div>").parent().addClass(wijCSS.helperHidden), container = selectWrap.wrap("<div></div>").parent().attr("role", "listbox").addClass(wijCSS.wijdropdown).addClass(wijCSS.widget).addClass(wijCSS.stateDefault).addClass(wijCSS.cornerAll).addClass(wijCSS.helperClearFix), label = $("<label></label>").addClass(wijCSS.wijdropdownLabel).addClass(wijCSS.cornerAll).attr("id", ele.get(0).id + "_select"), rightTrigger = $("<div></div>").addClass(wijCSS.wijdropdownTrigger).addClass(wijCSS.stateDefault).addClass(wijCSS.cornerRight), labelWrap = $("<a href=\"#\"></a>"), listContainer = $("<div>").addClass(wijCSS.wijmodropdown), list = $("<ul></ul>").addClass(wijCSS.wijdropdownList).addClass(wijCSS.content).addClass(wijCSS.widget).addClass(wijCSS.cornerAll).addClass(wijCSS.helperReset).appendTo(listContainer);
            ...
            wijdropdown.prototype._buildList = function (list, listContainer, eleWidth) {
                var self = this, wijCSS = self.options.wijCSS, ele = self.element, height, listWidth;
                ...
                // Get the original width of list
                list.width("").css("float", "left");
                ...
                if (this.cssWidth === "" || this.cssWidth === "auto") {
                    if (listWidth > eleWidth - 19) {
                        eleWidth = listWidth + 10;
            ...
            wijdropdown.prototype._toggleStateClass = function (cssName, toggle) {
                this._label.toggleClass(cssName, toggle);
                this._rightTrigger.toggleClass(cssName, toggle);
                this._container.toggleClass(cssName, toggle);
            };
            ...
            wijdropdown.prototype._handelEvents = function (ele) {
                ...
                ele.bind("click" + namespace, function (e) {
                    ...
                    self._toggleStateClass(wijCSS.stateHover, true);
                    element.trigger('mouseover');
                }).bind("mouseout" + namespace, function () {
                    if (self._isDisabled()) {
                        return;
                    }
                    self._toggleStateClass(wijCSS.stateHover, false);
                    element.trigger('mouseout');
                }).bind("mousedown" + namespace, function () {
                    if (self._isDisabled()) {
                        return;
                    }
                    self._toggleStateClass(wijCSS.stateActive, true);
                    element.trigger('mousedown');
                }).bind("mouseup" + namespace, function () {
                    if (self._isDisabled()) {
                        return;
                    }
                    self._toggleStateClass(wijCSS.stateActive, false);
            ...
            wijdropdown.prototype._bindEvents = function () {
                ...
                $(window).bind("resize", function (e) {
                    ...
                    if (listContainer.is(":hidden")) {
                        self._toggleStateClass(wijCSS.stateFocus, false);
                        return;
                    }
                ...
                labelWrap.bind("keydown" + namespace, function (e) {
                    ...
                    self._toggleStateClass(wijCSS.stateFocus, true);
                    ele.focus();
