        ...
        var wijlist = (function (_super) {
            ...
            */
            ...
            wijlist.prototype._setOption = function (key, value) {
                var self = this, o = this.options, ulOuterHeight, isBind, renderItems, selectedItem;
                ...
                if (key === "selectionMode") {
                    ...
                    if (selectedItem) {
                        ...
                        if (selectedItem.element) {
                            self._removeActiveClasses(selectedItem.element);
                        }
                    ...
                    if (self.selectedItems) {
                        $.each(self.selectedItems, function (index, i) {
                            i.selected = false;
                            self._removeActiveClasses(i.element);
                        });
                    }
                    self.selectedItems = [];
                    if (value === "single") {
            ...
            wijlist.prototype._create = function () {
                var self = this, ele = this.element, o = this.options, wijCSS = o.wijCSS, listCSS = wijCSS.widget + " " + wijCSS.content + " " + wijCSS.cornerAll + " wijmo-wijlist", renderItems, ulContentContainer;
                ...
                ele.addClass(listCSS).bind("click." + self.widgetName, self, self._onListClick).bind("mousedown." + self.widgetName, self, self._onListMouseDown);

                if (ele.is("div") && ele.children().is("ul")) {
                    self._isInnerData = true;
                    self._templates = [];
                    ulContentContainer = ele.children("ul");
                    $.each(ulContentContainer.children("li"), function (idx, liNode) {
                        self._templates.push({ templateHtml: liNode.innerHTML });
                    });

                    self._oriChildren = ulContentContainer.hide();
                }
                ...
                if ((o.listItems && o.listItems.length > 0) || (self.items && self.items.length > 0)) {
                    ele.attr({
                        role: "listbox",
            ...
            */
            ...
            wijlist.prototype._removeActiveClasses = function (item) {
                var activeClass = listItemCSSSelected + " " + this.options.wijCSS.stateActive;
                item.removeClass(activeClass + " wijmo-wijcombobox-selecteditem");
            };
                ...
                if (!itemsChanged && (!searchTerm || searchTerm.length === 0)) {
                    $.each(this.items, function (index, item) {
                        if (item.selected) {
                            ...
                            self._removeActiveClasses(item.element);
                        }
                    });
                    return;
                }
            ...
            */
            wijlist.prototype.destroy = function () {
                ...
                ele.removeClass("wijmo-wijobserver-visibility").removeClass(listCSS).removeAttr("role").unbind("." + self.widgetName);
                self.ul.remove();
            ...
            wijlist.prototype.select = function (event, data) {
                ...
                if (singleMode) {
                    ...
                    if (previous !== undefined && item !== previous) {
                        ...
                        if (previous.element) {
                            self._removeActiveClasses(previous.element);
                            previous.element.removeAttr("aria-selected");
                        }
                    }
                    self.selectedItem = item;
                    ...
                    if (item.selected) {
                        ...
                        self._removeActiveClasses(ele);
                        ele.removeAttr("aria-selected", "true");
            ...
            */
            wijlist.prototype.refreshSuperPanel = function () {
                ...
                if (self._isDisabled() && self.disabledDiv) {
