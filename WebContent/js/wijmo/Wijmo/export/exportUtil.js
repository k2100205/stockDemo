...
(function (wijmo) {
    (function (exporter) {
        ...
        function _formatUint(uint, length) {
        ...
        function _localDateTimeToJSON() {
            var date = this;
            return _formatUint(date.getFullYear(), 4) + '-' + _formatUint(date.getMonth() + 1, 2) + '-' + _formatUint(date.getDate(), 2) + 'T' + _formatUint(date.getHours(), 2) + ':' + _formatUint(date.getMinutes(), 2) + ':' + _formatUint(date.getSeconds(), 2) + "." + _formatUint(date.getMilliseconds(), 3);
        }
        ...
        (function (ExportMethod) {
            ...
            ExportMethod[ExportMethod["Content"] = 0] = "Content";

            /** Sending widget options to the service for exporting. */
            ExportMethod[ExportMethod["Options"] = 1] = "Options";
        })(exporter.ExportMethod || (exporter.ExportMethod = {}));
        var ExportMethod = exporter.ExportMethod;
