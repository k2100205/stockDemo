...
(function (wijmo) {
    (function (upload) {
        ...
        var wijupload = (function (_super) {
                ...
                return {
                    onSelect: function (file) {
                        ...
                        if (widget._trigger("change", null, file) === false) {
                            // NOTE: cancelUpload() only dequeue one file, not clear the whole queue.
                            swfupload.cancelUpload(file.id);
                            return false;
                        }
                        widget._createFileRow(file);
                        widget._setAddBtnState();
            ...
            wijupload.prototype._innerDisable = function () {
                _super.prototype._innerDisable.call(this);
                this._handleDisabledOption(true, this.upload);
            };

            wijupload.prototype._innerEnable = function () {
                _super.prototype._innerEnable.call(this);
                this._handleDisabledOption(false, this.upload);
            };
            ...
            */
            wijupload.prototype.destroy = function () {
                ...
                if (self.swfupload) {
                    if (self.swfupload.destroy) {
                        self.swfupload.destroy();
                    }
                    self.swfupload = null;
                }
            ...
            */
            ...
            wijupload.prototype._createFileInput = function () {
                ...
                fileInput.attr("aria-label", "wijupload");
            ...
            wijupload.prototype._setAddBtnState = function () {
                var self = this, o = self.options, maxFiles = self.options.maximumFiles || self.maxDisplay, addBtn = self.addBtn, files;
                if (!maxFiles) {
                    return;
                }
                if (!addBtn) {
                ...
                if (files.length >= maxFiles) {
                    ...
                    self.maskDiv.show();
                    if ((o.enableSWFUploadOnIE || o.enableSWFUpload) && $.browser.msie && parseInt($.browser.version) <= 10) {
                        self.maskDiv.bgiframe({ opacity: true, conditional: true });
                    }
                    if (self.input) {
                    ...
                    self.maskDiv.hide();
                    if ((o.enableSWFUploadOnIE || o.enableSWFUpload) && $.browser.msie && parseInt($.browser.version) <= 10) {
                        self.maskDiv.find("iframe.bgiframe").remove();
                    }
                }
            };
            ...
            wijupload.prototype._bindEvents = function () {
                ...
                self.upload.delegate(isUploadUpload, "click." + self.widgetName, function (e) {
                    ...
                    if ((o.enableSWFUploadOnIE && $.browser.msie) || o.enableSWFUpload) {
                        ...
                        self._removeFileRowSetButtonStates();
                    } else {
                        fileInput = $("input", fileRow[0]);
                        uploader = self.uploaders[fileInput.attr("id")];
            ...
            wijupload.prototype._removeFileRowSetButtonStates = function () {
                var self = this;
                $(self).parents(isUploadFileRow).remove();
                self._setAddBtnState();
                if ($(isUploadFileRow, self.element).length === 0) {
                    self.commandRow.hide();
                }
            };
